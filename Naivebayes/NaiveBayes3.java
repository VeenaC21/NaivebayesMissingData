...
public class NaiveBayes3 {
	...
	static double[][] marginalcount = new double[17][2];
	...
	public static void main(String[] args) throws NumberFormatException,
		...
		first: while ((line = br.readLine()) != null) {
	...
	public static void naiveBayes() {
		...
		// calculate mode for each attribute
		int county = 0;
		int countn = 0;
		...
		for (int j = 1; j <= 16; j++) {
			...
			marginalcount[j][0] = county;
			marginalcount[j][1] = countn;
			county = 0;
			countn = 0;
		}

		//fill the missing values for training data
		for (int i = 0; i <= 265; i++)  {

			for (int j = 1; j <= 16; j++){

				if(traininput[i][j]=='?'){
					if(marginalcount[j][0]>marginalcount[j][1]){
						traininput[i][j]='y';
					}
					else
						traininput[i][j]='n';
		...
		/*
		 * double product1=1;//republic double product2=1;//democratic for (int
		 * i = 0; i <= 168; i++) {
		 * 
		 * product1*=1/(double)countR; product2*=1/(double)countD; for (int j =
		 * 1; j <= 16; j++){
		 * 
		 * 
		 * if(test[i][j]=='y'){ product1*=marginalProb[j][0];
		 * product2*=marginalProb[j][2]; } else if(test[i][j]=='n'){
		 * product1*=marginalProb[j][1]; product2*=marginalProb[j][3]; }
		 * 
		 * 
		 * }
		 * 
		 * if(product1>product2){ testResult[i]='r'; } else testResult[i]='d';
		 * 
